...
class WebSocketConfigTest {
    ...
    void shouldRegisterWebSocketHandlers() {
        ...
        when(registry.addHandler(any(TextWebSocketHandler.class), eq("/ws/agent-stream"))).thenReturn(registration);
        when(registration.setAllowedOrigins("*")).thenReturn(registration);

        config.registerWebSocketHandlers(registry);

        verify(registry).addHandler(any(TextWebSocketHandler.class), eq("/ws/agent-stream"));
